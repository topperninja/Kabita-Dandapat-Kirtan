...
class CppAdapter : LanguageAdapter {
    override val id = "cpp"
    override val name = "C++"
    override val description = "C++17 GCC/Clang Toolchain"
    override val version = "17.0.0"
    override val supportedExtensions = listOf(".cpp", ".cc", ".cxx", ".hpp", ".h")
    ...
    private fun getCompilerPaths(context: Context): List<String> {
        val appToolchain = File(context.filesDir, "toolchains/cpp/bin/clang++").absolutePath
        return listOf(
            appToolchain,
    ...
    override suspend fun checkHealth(context: Context, onLog: (ConsoleMessage) -> Unit): Boolean = withContext(Dispatchers.IO) {
        ...
        val testFile = File(context.cacheDir, "main.cpp").apply {
            writeText("#include <iostream>\nint main() { std::cout << \"CLASSMASTI_CPP_OK\" << std::endl; return 0; }")
        }
        ...
        if (result.state == ExecutionState.SUCCESS) {
            onLog(ConsoleMessage("[$name] Offline C++17 toolchain is READY.", ConsoleMessageType.SYSTEM))
            return@withContext true
        }
    ...
    override suspend fun build(context: Context, projectDir: File, onLog: (ConsoleMessage) -> Unit): Boolean = withContext(Dispatchers.IO) {
        val nativeCompiler = getValidCompiler(context)
        if (nativeCompiler != null) {
            val cppFiles = projectDir.listFiles { _, name -> name.endsWith(".cpp") || name.endsWith(".cc") }?.toList() ?: emptyList()
            if (cppFiles.isEmpty()) {
                onLog(ConsoleMessage("Error: No .cpp source files found.", ConsoleMessageType.ERROR))
                return@withContext false
            }
            val outFile = File(projectDir, "a.out")
            val cmd = mutableListOf(nativeCompiler, "-std=c++17")
            cmd.addAll(cppFiles.map { it.name })
            cmd.add("-o")
    ...
    override suspend fun run(
        ...
        usingNative = false
        val target = if (mainFile.exists()) mainFile else projectDir.listFiles { _, name -> name.endsWith(".cpp") }?.firstOrNull()
        if (target == null) {
            val err = "Error: No .cpp source file found."
